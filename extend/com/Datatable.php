...
class Datatable{
	...
	public function create_status(){
		$this->sql .= "`status` tinyint(2) NOT NULL DEFAULT '0' COMMENT '状态',";
		return $this;
	}
	...
	public function colum_field($table,$attr = array()){
		...
		if(substr($field_attr['type'], 0, 2) == 'un'){
			$field_attr['type'] = substr($field_attr['type'], 2);
			$field_attr['unsigned'] = 'unsigned';
		}else{
			$field_attr['unsigned'] = '';
		}
		...
		$field_attr['default'] = $attr['default'] !== '' ? 'DEFAULT \''.$attr['default'].'\'' : 'DEFAULT \'\'';
		if($field_attr['is_null'] == 'null' && $field_attr['default'] == 'default ""'){
			$field_attr['default'] = '';
		}
		...
		if($field_attr['action'] == 'ADD'){
			$this->sql = "ALTER TABLE `{$field_attr['table']}` ADD `{$field_attr['field']}` {$field_attr['type']}{$field_attr['length']} {$field_attr['unsigned']} {$field_attr['is_null']} {$field_attr['default']} COMMENT '{$field_attr['comment']}' {$field_attr['after']}";
		}elseif($field_attr['action'] == 'CHANGE'){
			$this->sql = "ALTER TABLE `{$field_attr['table']}` CHANGE `{$field_attr['oldname']}` `{$field_attr['newname']}` {$field_attr['type']}{$field_attr['length']} {$field_attr['unsigned']} {$field_attr['is_null']} {$field_attr['default']} COMMENT '{$field_attr['comment']}' {$field_attr['after']}";
		}
		return $this;
	}
