...
// +----------------------------------------------------------------------
// | 来源: https://github.com/5ini99/think-auth
// +----------------------------------------------------------------------
namespace com;
...
class Auth
{
     ...
     */
    public function check($name, $uid, $relation = 'or', $type = 0, $mode = 'url')
    {
    ...
    public function getGroups($uid)
    {
        ...
        $user_groups = Db::view($auth_group_access, 'uid,group_id')
            ...
            ->where("{$auth_group_access}.uid='{$uid}' and {$auth_group}.state='1'")
            ->select();
        */
        $user_groups = Db::view($auth_group, 'id as group_id,title,rules')
            ->where("FIND_IN_SET({$uid}, users) and state='1'")
            ->select();
