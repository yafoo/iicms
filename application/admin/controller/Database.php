...
class Database extends Controller
{
	...
	public function index($type = null) {
		...
		case 'import':
			//列出备份文件列表
			$path = config('sys.data_backup_path'); 
			if (!is_dir($path)) {
	...
	public function del($time = 0) {
		if ($time) {
			...
			$path = realpath(config('sys.data_backup_path')) . DIRECTORY_SEPARATOR . $name;
	...
	public function export($tables = null, $id = null, $start = null) {
		if (request()->isPost() && !empty($tables) && is_array($tables)) {
			...
			//读取备份配置
			$config = array('path' => realpath($path) . DIRECTORY_SEPARATOR, 'part' => 1024 * config('sys.data_backup_part_size'), 'compress' => config('sys.data_backup_compress'), 'level' => config('sys.data_backup_compress_level'));
			//检查是否有正在执行的任务
	...
	public function import($time = 0, $part = null, $start = null) {
		if (is_numeric($time) && is_null($part) && is_null($start)) { 
			...
			$db = new \com\Database($list[$part], array('path' => realpath(config('sys.data_backup_path')) . DIRECTORY_SEPARATOR, 'compress' => $list[$part][2]));
